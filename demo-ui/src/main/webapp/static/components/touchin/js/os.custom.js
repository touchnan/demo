...
function renderInfoTab($tab,oData){
...
function preProcessAjaxData(data, invoker) {
	...
	if (data && (data.err)) {
		...
		if (data.errMsg || data.errCode) {
			(invoker || showMsg) (data.errMsg || data.errCode);
		}
...
function jqGridFormSearch($jqGrid, $jqForm) {
	$jqGrid.jqGrid('setGridParam',{postData:{ qtype:DEFAULT_QTYPE, query: $jqForm.serialize()} }).trigger("reloadGrid");	
}
        ...
        $.ajax({
            ...
            success: function(data,result) {
                ...
                for (var i=data.length; i--;) {
                    r.push("{0}:{1}".format(data[i].id, (data[i].nickname || data[i].realname || data[i].loginName)));
                }
                options = r.join(";")
                //if (!result) alert('Failure to ajax retrieve.');
            }
...
function bindData2Select(obj, aParis) {
...
function bindDetailSelect(url,obj) {
	try {
		var data = $.parseJSON($.ajax({
		    url: url,
		    async: false,
...
function bindDetailData2Select(obj, aParis) {
	var optionsObj = obj.options;
	optionsObj.length=0;
	if (aParis) {
		var nstr = "{0} ({1})";
		for (var i=aParis.length; i--;) {
			var name = aParis[i].name;
			var value = aParis[i].value;
			optionsObj.add(new Option(nstr.format(name,value),value));
		}
	}
}
		...
		if ( status >= 200 && status < 300 || status === 304 ) {
			if ( status === 304 ) {
				...
				try {
					...
					if (data && (data.err)) {
						if (data.errMsg || data.errCode) {
							var eMsg = (data.errMsg || data.errCode);
							return [false, eMsg, -1];
						}
						if(data.result && data.result.url) {
							window.top.location.href=data.result.url;
...
(function($) {
	function gridParam() {
	    ...
	    return $.extend({
	        ...
	        datatype: "json",
	        rowNum: 15,
	        rowList: [10, 15, 20, 30, 50, 100,10000],
	        pager: '#pager',
	        //sortname: 'id',
	        //sortorder: "desc",
	        viewrecords: true,
