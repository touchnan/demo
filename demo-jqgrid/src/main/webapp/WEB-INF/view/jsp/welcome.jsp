	...
	$(document).ready(function() {
 	...
 	function msp() {
		...
		if ($("#newPasswd").val() != $("#surePasswd").val()) {
 		...
 		$.ajax({
    		url: '<c:url value='/user/msp'/>'+getTimestamp('?'),
    		type: 'POST',
    		dataType:'json',
...
<div class="ui-layout-center">
			...
			<tr>
				...
				<td style="padding-right:5px;text-align:right;vertical-align:bottom;">
					<div id="touchin-topmenu">
						<span style="color:#fff;text-decoration:none">欢迎您,<span style="font-size:22px;font-weight:bold;color:black;">${me.loginName}</span></span>
						<%--<a href="<%=ctn%>">主页</a>--%>
						<a href="#" onclick="passwd()">密码</a>
						<a href="<c:url value='/manage/logout'/>">退出</a>
					</div>
				</td>
			</tr>
		</table>	
	...
	<div class="ui-layout-west">
		<div id="mm" class="easyui-accordion" border="false">
			<div title="代理账号与库存管理">
				<ul class="pitem">
					<li><a href="javascript:void(0)" onclick="open1('<c:url value="/user/users"/>')">代理信息</a></li>
					<%--<li><a href="javascript:void(0)" onclick="open1('<c:url value="/card/storage"/>')">代理加卡</a></li>--%>
				</ul>
			</div>
<%--			<div title="促销活动管理">
				<ul class="pitem">
					<li><a href="javascript:void(0)" onclick="open1('<c:url value="/card/cards"/>')">卡活动</a></li>
				</ul>
			</div>
			<div title="统计查询分析">
				<ul class="pitem">
					<li><a href="javascript:void(0)" onclick="open1('<c:url value="/card/report_detail/10"/>')">明细</a></li>
					<li><a href="javascript:void(0)" onclick="open1('<c:url value="/card/report_stat/10"/>')">统计</a></li>
				</ul>
			</div>
			<div title="交易查询">
				<ul class="pitem">
					<li><a href="javascript:void(0)" onclick="open1('<c:url value="/query/query/cardOrders"/>')">购卡交易单</a></li>
				</ul>
			</div>	--%>
		</div>
	</div>
		...
		<ul class="list def-list">
			<li>
				<label>
					...
					<input type="password" id="oldPasswd" name="passwd"/>
				</label>
			</li>		
			<li>
				<label>
			...
			<li>
				<label>
					...
					<input type="password" id="surePasswd" name="surePasswd"/>
				</label>
			</li>	
		</ul>
	</form>
