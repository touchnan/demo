...
<script type="text/javascript">
    ...
    $(document).ready(function () {
       ...
       /* var userRoles = jqSelect('users/enum!userRoles.action'),
        ...
        $("#grid").bindGrid({
            _oname:"user",
            url:"<c:url value='/card/report_list_stat/'/>${proxy_type}",
            pager: '#pager',
            sortname: 'id',
            sortorder: "desc",
            caption:"购卡统计",
            ...
            colModel:[
                ...
                {name:'buyDate',label:'购买日期',index:'T_col_buy_time',align:'center',
                    //hidedlg:true,
                    search:true,
                    editable: false,
                    formatter: function (cellvalue, options, rowObject) {
    ...
    $('#proxyType').on("change",function(){
        ...
        grid.setGridParam({url:"<c:url value='/card/report_list_stat/'/>"+$('#proxyType').val()}).trigger("reloadGrid",[{page:1,filters:""}])
	});
